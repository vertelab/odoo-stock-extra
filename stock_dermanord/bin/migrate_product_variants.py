...
oerp = oerplib.OERP(local_server, protocol='xmlrpc', port=local_port)

# Check available databases
print(oerp.db.list())

# Login (the object returned is a browsable record)
user = oerp.login(local_user, local_passwd, local_database)
print(user.name)            # name of the user connected
print(user.company_id.name) # the name of its company

#for template in oerp.get('product.template').browse(oerp.get('product.template').search([('','','')])):
#for template in oerp.get('product.template').browse(oerp.get('product.template').search([])):
for template in oerp.get('product.template').search([])[:50]:
    print(template)
    record = oerp.get('product.template').read(template,['name'])
